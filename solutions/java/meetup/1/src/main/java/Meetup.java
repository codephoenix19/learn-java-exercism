...
class Meetup {
    ...
    Meetup(int monthOfYear, int year) {
        try {
            if(monthOfYear > 12 || monthOfYear < 1 || year < 1900 || year > 2026) {
                throw new UnsupportedMeetupOperation("Invalid input : month or year");
            }
            this.monthOfYear = monthOfYear;
            this.year = year;
        } catch (UnsupportedMeetupOperation ex) {
    ...
    LocalDate day(DayOfWeek dayOfWeek, MeetupSchedule schedule) {
        YearMonth yearMonth = YearMonth.of(this.year, this.monthOfYear);
        LocalDate current = yearMonth.atDay(1);
        List<LocalDate> matchingDays = new LinkedList<>();

        while (current.getMonthValue() == this.monthOfYear) {
        ...
        return switch (schedule) {
            ...
            case TEENTH ->
                // week = x
                    matchingDays.stream().filter(date -> date.getDayOfMonth() >= 13 && date.getDayOfMonth() <= 19).findFirst().orElseThrow(() -> new IllegalStateException());
        };
    }
